#!/usr/bin/env python3
"""
Instagram Scraper - Versão Ordenada por Data
Coleta reels dos mais novos para os mais antigos
"""
import json
import time
import random
...
class InstagramScraperReelsOrdered:
    """
    Instagram Scraper que coleta reels dos mais novos para os mais antigos
    """
    ...
    def _collect_posts_minimal(self, profile, target_posts: int) -> List:
        """Coleta posts de forma mínima - apenas para ter dados básicos"""
        posts = []
        consecutive_data_errors = 0
        max_data_errors = 2
        
        try:
            print(f"📊 Coletando posts básicos (objetivo: {target_posts})...")
    ...
    def _collect_reels_ordered(self, profile, min_reels: int = 12) -> List:
        """Coleta reels dos mais novos para os mais antigos"""
        reels_data = []
        consecutive_data_errors = 0
        max_data_errors = 10
        ...
        try:
            print(f"📹 Coletando reels ordenados (objetivo: {min_reels}) - MAIS NOVOS PARA MAIS ANTIGOS...")
            posts_generator = profile.get_posts()
            ...
            while len(reels_data) < min_reels and post_count < max_posts_to_check:
                try:
                    ...
                    if getattr(post, 'typename', '') == 'GraphVideo':
                        ...
                        # Formatar views para exibição
                        views_formatted = self._format_number(reel_data['views'])
                        likes_formatted = self._format_number(reel_data['likes'])
                        comments_formatted = self._format_number(reel_data['comments'])
                        
                        print(f"   📹 Reel {len(reels_data)}: {likes_formatted} likes, {views_formatted} views, {comments_formatted} comments")
                        print(f"       Data: {reel_data['date']}")
                        print(f"       Shortcode: {reel_data['shortcode']}")
    ...
    def _format_number(self, number: int) -> str:
        """Format number for display (e.g., 1000000 -> 1.0M)"""
        if number >= 1000000:
    ...
    def get_profile_data(self, username: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Coletar posts básicos (3x mais, mas com foco em reels)
            ...
            min_reels = 12
            reels_data = self._collect_reels_ordered(profile, min_reels)
            ...
            if len(reels_data) < min_reels:
                ...
                additional_reels = self._collect_reels_ordered(profile, min_reels - len(reels_data))
    ...
    def scrape_user(self, username: str) -> Dict:
        ...
        """
        print(f"🚀 Iniciando scraping ordenado de reels: {username}")
        logger.info(f"Starting ordered reels scrape for user: {username}")
...
def main():
    ...
    print("🎯 Instagram Scraper - Reels Ordenados - Iniciando...")
    print("=" * 60)
    
    scraper = InstagramScraperReelsOrdered()
