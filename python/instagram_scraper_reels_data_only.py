#!/usr/bin/env python3
"""
Instagram Scraper - Versão Focada em Dados de Reels
Coleta apenas os dados essenciais (likes, comments, views) dos reels
"""
import json
import time
import random
...
class InstagramScraperReelsDataOnly:
    """
    Instagram Scraper focado apenas em dados de reels
    """
    ...
    def _collect_reels_data_only(self, profile, min_reels: int = 12) -> List:
        """Coleta apenas dados essenciais dos reels - sem acessar conteúdo completo"""
        reels_data = []
        consecutive_data_errors = 0
        max_data_errors = 10  # Mais tolerante para reels
        post_count = 0
        max_posts_to_check = 500  # Aumentar muito o limite
        
        try:
            print(f"📹 Coletando dados de reels (objetivo: {min_reels}) - APENAS DADOS ESSENCIAIS...")
            posts_generator = profile.get_posts()
            ...
            while len(reels_data) < min_reels and post_count < max_posts_to_check:
                try:
                    ...
                    if getattr(post, 'typename', '') == 'GraphVideo':
                        ...
                        print(f"   📹 Reel {len(reels_data)}: {reel_data['likes']} likes, {reel_data['views']} views, {reel_data['comments']} comments")
                        consecutive_data_errors = 0
    ...
    def get_profile_data(self, username: str) -> Optional[Dict]:
        ...
        try:
            ...
            min_reels = 12
            reels_data = self._collect_reels_data_only(profile, min_reels)
            ...
            if len(reels_data) < min_reels:
                ...
                additional_reels = self._collect_reels_data_only(profile, min_reels - len(reels_data))
    ...
    def scrape_user(self, username: str) -> Dict:
        ...
        """
        print(f"🚀 Iniciando scraping focado em dados de reels: {username}")
        logger.info(f"Starting reels-data-only scrape for user: {username}")
...
def main():
    ...
    print("🎯 Instagram Scraper - Dados de Reels Apenas - Iniciando...")
    print("=" * 60)
    
    scraper = InstagramScraperReelsDataOnly()
