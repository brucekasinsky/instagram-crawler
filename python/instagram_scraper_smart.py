#!/usr/bin/env python3
"""
Instagram Scraper - Versão Inteligente
Para quando encontra erro 'data' e coleta o máximo possível
"""
import json
import time
import random
...
class InstagramScraperSmart:
    """
    Instagram Scraper inteligente que para quando encontra erro 'data'
    """
    ...
    def _collect_posts_smart(self, profile, target_posts: int) -> List:
        """Coleta posts de forma inteligente, para quando encontra erro 'data'"""
        posts = []
        consecutive_data_errors = 0
        max_data_errors = 3  # Para após 3 erros 'data' consecutivos
            ...
            while len(posts) < target_posts:
                try:
                    ...
                    if "'data'" in error_msg:
                        ...
                        print(f"⚠️ Erro 'data' {consecutive_data_errors}/3 - Parando coleta de posts")
                        
                        if consecutive_data_errors >= max_data_errors:
                            print("🛑 Muitos erros 'data' consecutivos, parando coleta de posts")
                            break
                    else:
                        print(f"⚠️ Erro no post {len(posts) + 1}: {error_msg}")
                        consecutive_data_errors = 0  # Reset para outros erros
    ...
    def _collect_reels_smart(self, profile, min_reels: int = 12) -> List:
        """Coleta reels de forma inteligente"""
        reels = []
        consecutive_data_errors = 0
        max_data_errors = 3
        
        try:
            print(f"📹 Coletando reels (objetivo: {min_reels})...")
            posts_generator = profile.get_posts()
            post_count = 0
            max_posts_to_check = 200  # Aumentar limite para encontrar mais reels
            ...
            while len(reels) < min_reels and post_count < max_posts_to_check:
                try:
                    ...
                    if getattr(post, 'typename', '') == 'GraphVideo':
                        ...
                        print(f"   📹 Reel {len(reels)}: {likes} likes, {views} views")
                        consecutive_data_errors = 0  # Reset contador
                    ...
                    if "'data'" in error_msg:
                        consecutive_data_errors += 1
                        print(f"⚠️ Erro 'data' {consecutive_data_errors}/3 na coleta de reels")
    ...
    def get_profile_data(self, username: str) -> Optional[Dict]:
        ...
        try:
            ...
            posts = self._collect_posts_smart(profile, target_posts)
            print(f"✅ Coletados {len(posts)} posts (objetivo: {target_posts})")
            
            # Coletar reels (mínimo 12)
            min_reels = 12
            reels = self._collect_reels_smart(profile, min_reels)
            print(f"✅ Coletados {len(reels)} reels (objetivo: {min_reels})")
    ...
    def scrape_user(self, username: str) -> Dict:
        ...
        """
        print(f"🚀 Iniciando scraping inteligente para usuário: {username}")
        logger.info(f"Starting smart scrape for user: {username}")
...
def main():
    ...
    print("🎯 Instagram Scraper Inteligente - Iniciando...")
    print("=" * 50)
    
    scraper = InstagramScraperSmart()
