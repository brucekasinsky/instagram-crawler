#!/usr/bin/env python3
"""
Instagram Scraper - Versão Otimizada para Reels
...
class InstagramScraperReelsOptimized:
    ...
    def _collect_posts_optimized(self, profile, target_posts: int) -> List:
        ...
        max_data_errors = 2  # Reduzir para parar mais rápido
                ...
                try:
                    ...
                    if "'data'" in error_msg:
                        ...
                        if consecutive_data_errors >= max_data_errors:
                            print("🛑 Parando coleta de posts para focar nos reels")
                            break
                    else:
    ...
    def _collect_reels_optimized(self, profile, min_reels: int = 12) -> List:
        ...
        try:
            print(f"📹 Coletando reels (objetivo: {min_reels}) - FOCO PRINCIPAL...")
            posts_generator = profile.get_posts()
            ...
            while len(reels) < min_reels and post_count < max_posts_to_check:
                try:
                    ...
                    if getattr(post, 'typename', '') == 'GraphVideo':
                        ...
                        print(f"   📹 Reel {len(reels)}: {likes} likes, {views} views, {comments} comments")
                        consecutive_data_errors = 0  # Reset contador
                        
                        # Log de progresso a cada 5 reels
                        if len(reels) % 5 == 0:
                            print(f"   🎯 Progresso: {len(reels)}/{min_reels} reels coletados")
                    ...
                    if "'data'" in error_msg:
                        consecutive_data_errors += 1
                        print(f"⚠️ Erro 'data' {consecutive_data_errors}/5 na coleta de reels")
    ...
    def get_profile_data(self, username: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Coletar reels (mínimo 12) - FOCO PRINCIPAL
            min_reels = 12
            ...
            if len(reels) < min_reels:
                print(f"⚠️ Apenas {len(reels)} reels encontrados, tentando mais uma vez...")
                additional_reels = self._collect_reels_optimized(profile, min_reels - len(reels))
                reels.extend(additional_reels)
                print(f"✅ Total de reels coletados: {len(reels)}")
    ...
    def scrape_user(self, username: str) -> Dict:
        ...
        """
        print(f"🚀 Iniciando scraping otimizado para reels: {username}")
...
def main():
    ...
    print("🎯 Instagram Scraper Otimizado para Reels - Iniciando...")
    print("=" * 60)
